...
**Token Economy.** The per-change token spend pattern of an AI-assisted engineering operation. In Spec-Driven Development without a structured substrate, token spend tends to grow super-linearly with change complexity because the agent has to receive enough prompt-context to cover every case it might encounter; richer changes mean richer prompts mean more iteration loops. In Semantic Engineering, the agent queries the knowledge graph for the slice each task needs and the cost per invocation tracks change size rather than surrounding context. The fifth ceiling condition that surfaces as teams mature past Zone 2. See [When This Zone Stops Working](../sdlc/zones/zone-2-spec-driven-development.md#when-this-zone-stops-working) and [Queryability and the Token Economy](../sdlc/methodology.md#queryability-and-the-token-economy).
